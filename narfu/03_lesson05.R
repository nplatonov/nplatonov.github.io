...
#' params:
#'   lesson: "03"
#' pagetitle: Занятие `r params$lesson`
...
#' mathjax: true
#' echo: true
#' title: Компоненты ГИС в R
#' subtitle: "![](assets/logo.jpg)"
...
#' shorttitle: "[WebR](https://webr.r-wasm.org/latest/)\n\n[Online Code Editor](https://rdrr.io/snippets/)"
#' ---
...
#' #### Базовый R
#'
#' Установить или обновить R, например, [отсюда](https://cran.rstudio.org/). Актуальная версия 4.3.1. Не ниже версии 4.1.
#'
#' #### Библиотеки
#'
#' :::scrollable
...
atSea <- Sys.Date()<as.Date("2023-08-07")
if (atSea)
   pkgList <- c("RColorBrewer","htmltools","DBI","Rcpp","sp","gtable","classInt"
               ,"e1071","crosstalk","rlang","R6","rlang","proxy","magrittr"
               ,"xfun","units","glue","evaluate","digest","fastmap"
               ,"htmlwidgets","base64enc","shiny","httpuv","later","lifecycle"
               ,"satellite","promises","leafsync"
               ,pkgList)
whoisready <- sapply(pkgList,function(pkg) {
   if (requireNamespace(pkg))
      return(TRUE)
   if (!atSea)
      install.packages(pkg,repos="https://cran.rstudio.com") ## хороший интернет
   else 
      install.packages(dir(path="D:/Product.ins/packages/zip4"
                          ,pattern=paste0("^",pkg,".+\\.zip")
                          ,full.names=TRUE)
                      ,repos=NULL,type="binary") ## Михаил Сомов
...
#'
#' Содержимое занятия сгенерировано из файла `03_lesson05.R`. Загрузим его, переименовав:
#+ download, eval=FALSE
sfile <- "03_lesson05.R"
rfile <- "03_lesson05-reproduced.R"
{
   if (file.exists(sfile))
      file.copy(sfile,rfile,overwrite=TRUE,copy.date=TRUE)
   else
      download.file(file.path("https://nplatonov.github.io/narfu"
                             ,gsub("^\\d+_","",sfile)),rfile)
}
