...
tm_shape(provinces_VIE_iso)+tm_fill(col = "name", alpha = 0.8, legend.show = FALSE)+tm_polygons(col = "white", alpha = 0.1, legend.show = FALSE)+tm_text("name_en",size = 0.25, alpha = 0.6)+tm_shape(seas)+tm_fill(col = "blue", alpha = 0.2, legend.show = FALSE)+tm_text("name_en", col = "white", alpha = 0.6, size = 0.4, ymod = -0.6)+tm_shape(roads)+tm_lines(col = "black", alpha = 0.2)+tm_shape(reefs)+tm_lines(col = "blue", alpha = 0.7)+ tm_shape(rivers_lakes)+tm_lines(col = "blue",alpha = 0.2)+tm_compass(type = "8star", size = 0.8, text.size = 0.5, show.labels = 2, position=c(0.8, 0))+tm_grid(alpha = 0.6)+tm_scale_bar(text.size = 15, width = 0.15, lwd = 0.25, position = c(0.016, 0.015))
