...
tm_shape(prov_AU_iso_MI) +
  ...
  tm_layout(fontface = "bold") + #(!) не получилось добавить белую обводку
  tm_borders(col="black", alpha = 0.01) +
    #добавим столицы штатов
    tm_shape(places_AU_cap) +
    tm_dots(group ="NAME", size = .1, alpha = 0.9) +
    tm_text("NAME", size =0.7,  remove.overlap = T, xmod = -.2, ymod =-.5,
            just = "left") + #(!) не получилось изменить шрифт и его font face 
  tm_legend()+
  #добавим рифы
  tm_shape(reefs) +
