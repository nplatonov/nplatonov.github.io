...
australia<-sf::st_read(file.path(root,"10m_physical","ne_10m_ocean.shp.zip"))
...
rivers.sf<-sf::st_read(file.path(root,"10m_physical","ne_10m_rivers_lake_centerlines.shp.zip"))
...
lakes<-sf::st_read(file.path(root,"10m_physical","ne_10m_lakes.shp.zip"))
lakes.sf <- lakes[st_is_valid(lakes),]
st_agr(lakes.sf) = "constant"

# Фильтрация регионов
states.sf<-sf::st_read(file.path(root,"10m_cultural","ne_10m_admin_1_states_provinces.shp.zip"))
...
# Фильтрация городов
cities.sf<-sf::st_read(file.path(root,"10m_cultural","ne_10m_populated_places.shp.zip"))
cities.au<-cities.sf[cities.sf$ADM0NAME=="Australia",]
cities.cap<-cities.au[cities.au$SCALERANK<4,]

# Операции с растром
ursa:::.elapsedTime("A1")
physical<-raster::stack(file.path(root,"../../naturalearth.raster","NE1_LR_LC.tif")) |> as("SpatRaster")
ursa:::.elapsedTime("A2")
check <- terra::rast(file.path(root,"../../naturalearth.raster","NE1_LR_LC.tif"))
ursa:::.elapsedTime("A3")
dim(physical)
dim(check)
identical(physical,check)
ursa:::.elapsedTime("A4")
