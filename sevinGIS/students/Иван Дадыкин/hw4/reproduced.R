...
shpname <- file.path(root,"10m_physical","ne_10m_ocean.shp")
ocean.sf <- ursa::spatial_read(shpname)
...
grat5 <- file.path(root,"10m_physical","ne_10m_graticules_5.shp")
grat5.sf <- ursa::spatial_read(grat5)
grat5_cropped<-sf::st_crop(grat5.sf, xmin=110, xmax=160, ymin=-45, ymax=-8)

rivers <- file.path(root,"10m_physical","ne_10m_rivers_lake_centerlines.shp")
rivers.sf <- ursa::spatial_read(rivers)
rivers_cropped<-sf::st_crop(rivers.sf, xmin=110, xmax=160, ymin=-45, ymax=-8)

lakes <- file.path(root,"10m_physical","ne_10m_lakes.shp")
lakes.sf <- ursa::spatial_read(lakes)
lakes_cropped<-sf::st_crop(lakes.sf, xmin=110, xmax=160, ymin=-45, ymax=-8)

states <- file.path(root,"10m_cultural","ne_10m_admin_1_states_provinces.shp")
states.sf <- ursa::spatial_read(states)
...
cities <- file.path(root,"10m_cultural","ne_10m_populated_places.shp")
cities.sf <- ursa::spatial_read(cities)
