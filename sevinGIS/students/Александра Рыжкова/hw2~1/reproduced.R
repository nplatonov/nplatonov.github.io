...
states <- sf::st_read(file.path(root, "10m_cultural", "ne_10m_admin_1_states_provinces.shp.zip"))
cities <- sf::st_read(file.path(root, "10m_cultural", "ne_10m_populated_places.shp.zip"))
oceans <- sf::st_read(file.path(root, "10m_physical", "ne_10m_ocean.shp.zip"))
marine_labels <- sf::st_read(file.path(root, "10m_physical", "ne_10m_geography_marine_polys.shp.zip"))
rivers <- sf::st_read(file.path(root, "10m_physical", "ne_10m_rivers_australia.shp.zip"))
#Для проверки данных:
#head(oceans)
#head(marine_labels)
