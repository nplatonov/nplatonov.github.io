...
dst_shp <- file.path(dir, "gis_data/ski_track_points.shp")
sf::st_write(
  obj = ski_table,
  dsn = dst_shp,
...
dst_shp <- file.path(dir, "gis_data/ski_track_points_time.shp")
           sf::st_write(
                        obj = ski_QGIS,
                        dsn = dst_shp,
                        driver = "ESRI Shapefile",
...
test <- st_read(file.path(dir, "gis_data/ski_track_points_time.shp"))
...
dst_geojson <- file.path(dir, "gis_data/ski_track_points.geojson")
sf::st_write(
  ski, 
  dsn = dst_geojson,
...
dst_geojson <- file.path(dir, "gis_data/ski_track_points_utm37.geojson")
sf::st_write(
  ski_QGIS, 
  dsn = dst_geojson,
  delete_dsn = TRUE 
...
dst_geojson <- file.path(dir, "gis_data/ski_track_line.geojson")
sf::st_write(
  ski_track_37N, 
  dsn = dst_geojson,
  delete_dsn = TRUE 
...
#для отката
dst_gpx <- file.path(dir, "gis_data/ski_track_processed.gpx")
sf::st_write(
  ski,
  dsn = dst_gpx,
...
list.files(file.path(dir, "gis_data"), pattern = "\\.geojson$|\\.shp$|\\.gpx$")
