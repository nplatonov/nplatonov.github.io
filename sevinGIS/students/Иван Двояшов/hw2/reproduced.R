...
require(mapview)
require(tidyverse) ## forgotten
gpx_data <- st_read("../../../data/2022-01-09_14-00_Sun.gpx", layer = "track_points") 
head(gpx_data)
gpx_data$time |> ursa::series()
...
trek
ld(trek) |> ursa::series() # get dataframe
plot(trek)
summary(trek)
...
plutil::pluglibrary(maptiles)  # For basemap tiles
...
basemap <- get_tiles(trek_ext, provider = "OpenStreetMap.DE", crop = TRUE, zoom = 16)
