...
# Preparing ---------------------------------------------------------------
loc <- st_read("2022-01-09_14-00_Sun.gpx", layer = "track_points") |>
  st_transform(32637)
...
#Средняя скорость в первом круге
speed_1 <- sapply(df_t1, mean, na.rm = TRUE)
speed_1[14]
# расчет по 2-му кругу
dist_2 <- df[["dist"]][395:690]
sum(dist_2)
# Время начала 2 круга
df[395,3]
# Время конца 2 круга
df[690,3]
...
#Средняя скорость во втором круге
speed_2 <-  sapply(df_t2, mean, na.rm = TRUE)
speed_2[14]
...
st_write(df_sf, "df.shp")

# Итоговая таблица
stats_1 <- tibble(
  Круг = "1",
  `Трасса, м` = sum(dist_1),
  `Средняя скорость, км/ч` = speed_1[14],
  Старт = df[94,3],
  Финиш = df[395,3])
stats_2 <- tibble(
  Круг = "2",
  `Трасса, м` = sum(dist_2),
  `Средняя скорость, км/ч` = speed_2[14],
  Старт = df[395,3],
  Финиш = df[690,3])
