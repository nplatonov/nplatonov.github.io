...
bathy_0 <- st_transform(bathy_0, aus_proj)
bathy_200 <- st_transform(bathy_200, aus_proj)
bathy_1000 <- st_transform(bathy_1000, aus_proj)
bathy_2000 <- st_transform(bathy_2000, aus_proj)
bathy_3000 <- st_transform(bathy_3000, aus_proj)
bathy_4000 <- st_transform(bathy_4000, aus_proj)
bathy_5000 <- st_transform(bathy_5000, aus_proj)
...
p <- ggplot() +
  ...
  annotation_custom(legend_grob, 
                    xmin = -2200000, xmax = -1600000, 
                    ymin = -4700000, ymax = -3700000) +
  ...
  coord_sf(xlim = c(-2200000, 2200000),
           ylim = c(-4800000, -800000),
           crs = aus_proj, label_graticule = "SW", label_axes = "--")

print(p)

  ggsave(filename = "D:/ИПЭЭ РАН/Аспирантура/QGIS/Australia_map_1.png",
