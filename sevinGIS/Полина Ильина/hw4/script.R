...
root<-"/cloud/project"
list.files()
baikal_polygon<- sf::st_read(file.path(root, "ne_10m_lakes.shp"))
land<- sf::st_read(file.path("ne_10m_admin_1_states_provinces.shp"))
river<-sf::st_read(file.path("rivers.shp"))
baikal_main_points<-sf::st_read(file.path("baikal_main_points.shp"))
sf_use_s2(FALSE)
